...
def train(args):
    ...
    model = BertDomainIntent.from_pretrained(args.pretrained, config=config, num_domains=num_domains,
                                             num_intents=num_intents, dropout=args.dropout,
                                             subspace=args.subspace, hierarchy=args.hierarchy,
                                             domain_first=args.domain_first)

    model.to(args.device)

    mtl_instance = MultiTaskLoss()
    mtl_instance.to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str, default="dataset/oos_full/", help="dataset path")
    ...
    parser.add_argument("--patience", type=int, default=3, help="number of more epochs before early stopping")
    parser.add_argument("--n_saved", type=int, default=1, help="number of models saved")
    parser.add_argument("--lr", type=float, default=4.00E-05, help="learning rate")
